...
class PyTextExtractor:
    ...
    def _extract_text(self, image, boxes, display, numbers, ratio_width, ratio_height):
        ...
        for (start_X, start_Y, end_X, end_Y) in boxes:
            ...
            # ratios
            # percentStart = (percentStart / 100) if percentStart >= 0 else ((-percentStart) / 100)
            # percentEnd = (percentEnd / 100 + 1) if percentEnd >= 0 else ((-percentEnd) / 100)

            start_X = int(start_X * ratio_width) - 15
            start_Y = int(start_Y * ratio_height) - 15
